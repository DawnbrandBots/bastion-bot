...
// 			await fetch(`${process.env.SEARCH_API}/yaml-yugi/multi?password=${allUniqueCards.join(",")}`)
// 		).json();
...
// 			if (c) {
// 				cardMemo.set(c.password ?? 0, c);
// 			}
// 		});
// 		// apply the names to the record of the deck
// 		// toString is fallback for missing name, e.g. if an API error returns null
// 		const getName = (password: number): string => cardMemo.get(password)?.name.en || password.toString();
// 		const namedDeck = {
